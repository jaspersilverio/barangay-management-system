...
import { 
  ...
  restoreCertificateRequest,
  getCertificateRequestsListCached,
  setCertificateRequestsListCached,
  clearCertificateRequestsListCache,
...
export default function CertificateRequests({ certificateType, archived = false }: CertificateRequestsProps = {}) {
  const { user } = useAuth()
  const canApprove = user?.role === 'captain' || user?.role === 'admin'
  const canManageRequests = ['staff', 'purok_leader', 'captain', 'admin'].includes(user?.role ?? '')
  ...
  const listKey = useMemo(
    () => `cert-requests:${archived ? 'archived:' : ''}${debouncedSearch}:${currentPage}:${statusFilter}:${typeFilter}`,
  ...
  const fetchRequests = useCallback(
    async (showLoading = true, cacheKeyArg?: string) => {
      ...
      try {
        ...
        if (!archived) {
          setCertificateRequestsListCached(k, { data: Array.isArray(list) ? list : [], last_page: lastPage })
        }
      } catch (err: unknown) {
  ...
  useEffect(() => {
    ...
    const cached = getCertificateRequestsListCached<{ data: CertificateRequestType[]; last_page: number }>(listKey)
    if (cached != null) {
    ...
    fetchRequests(true, listKey)
  }, [listKey, fetchRequests, archived])
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      const actionMessages = {
        ...
        delete: 'Record archived successfully',
  ...
  return (
    <div>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h5 className="mb-1 text-brand-primary">{archived ? 'Archived Certificate Requests' : 'Certificate Requests'}</h5>
          <p className="text-brand-muted mb-0">{archived ? 'View and restore archived requests' : 'Manage and track certificate requests from residents'}</p>
        </div>
        {!archived && (
      ...
      <div className="card-modern mb-4">
        <div className="row g-3">
          ...
          <div className="col-md-2">
            <ButtonComponent variant="outline" onClick={() => fetchRequests(true)}>
      ...
      <Modal show={showActionModal} onHide={() => setShowActionModal(false)}>
        <Modal.Header closeButton className="modal-header-custom">
          <Modal.Title className="modal-title-custom text-brand-primary">
            ...
            {actionType === 'delete' && 'Archive Request'}
            {actionType === 'restore' && 'Restore Request'}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body className="modal-body-custom">
          {selectedRequest && (
          ...
          {actionType === 'restore' && (
            <Alert variant="info">
              Restore this certificate request to the active list?
            </Alert>
          )}
        </Modal.Body>
        <Modal.Footer className="modal-footer-custom">
