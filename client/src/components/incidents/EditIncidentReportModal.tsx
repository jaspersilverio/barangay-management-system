...
const EditIncidentReportModal: React.FC<EditIncidentReportModalProps> = ({
  show,
  onHide,
  onSuccess,
  incidentReport
}) => {
  const [formData, setFormData] = useState<UpdateIncidentReportData>({
    incident_title: '',
    description: '',
    incident_date: '',
    incident_time: '',
  ...
  const populateForm = () => {
    ...
    const incidentDate = incidentReport.incident_date
      ? new Date(incidentReport.incident_date).toISOString().split('T')[0]
      : '';

    // Format time for input (HH:MM)
    const incidentTime = incidentReport.incident_time
      ? incidentReport.incident_time.substring(0, 5)
      : '';

    // Handle persons_involved (could be array or string)
    let personsInvolved = '';
    if (incidentReport.persons_involved) {
      if (Array.isArray(incidentReport.persons_involved)) {
        personsInvolved = incidentReport.persons_involved.join(', ');
      } else if (typeof incidentReport.persons_involved === 'string') {
        personsInvolved = incidentReport.persons_involved;
      }
    }

    setFormData({
      incident_title: incidentReport.incident_title || '',
  ...
  const handleInputChange = (field: keyof UpdateIncidentReportData, value: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!incidentReport || !validateForm()) {
      return;
    }

    try {
      setLoading(true);
      await updateIncidentReport(incidentReport.id, formData);
      onSuccess();
    } catch (error: any) {
      ...
      if (error.response?.data?.errors) {
        ...
        setErrors({ submit: error.response?.data?.message || 'Failed to update incident report.' });
      }
    } finally {
      setLoading(false);
    }
