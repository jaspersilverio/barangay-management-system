...
export default function Sidebar() {
    ...
    const checkItem = (
      ...
      if (item.children && item.children.length > 0) {
        const hasActiveChild = item.children.some((child) =>
          checkItem(
            child,
            item.isGroup ? parentLabels : [...parentLabels, item.label]
          )
        );
  ...
  const renderMenuItem = (
    ...
    if (level === 0 && shouldHighlightParent) {
      ...
      if (isParentExpanded) {
        ...
        activeParentLabel &&
        parentItem &&
        parentItem.label === activeParentLabel
      ) {
            ...
            >
              {item.children?.map((child) =>
                renderMenuItem(
                  ...
                  level + 1,
                  item,
                  isActiveTopLevelParent || isUnderActiveTree,
                  isExpanded
                )
              )}
            </div>
  ...
  return (
    <nav
      className="sidebar-fixed flex flex-col border-r shadow-sm d-none d-lg-flex"
    ...
    >
      ...
      >
        {visibleMenuItems.map((item) =>
          renderMenuItem(item, 0, undefined, false, false)
        )}
      </div>

      {/* Bottom Actions - Fixed at bottom */}
      <div className="p-4 pt-0 flex-shrink-0 border-t" style={{ borderColor: '#334155' }}>
        <Button
